...
@Schema(description = "Requête combinée pour créer un lien de paiement conditionnel (escrow) ET initier le paiement via YellowCard en un seul appel")
public class CreateConditionalAndPayYcRequest {
    ...
    @Schema(description = "Date d'expiration (optionnel, 24h par défaut)")
    ...
    @NotNull(message = "Le montant YellowCard est obligatoire")
    @Schema(description = "Montant à collecter via YellowCard (en devise locale)", example = "50000.0")
    private Double ycAmount;

    @NotBlank(message = "La devise YellowCard est obligatoire")
    @Schema(description = "Code devise YellowCard (ex: XOF, XAF, GHS)", example = "XOF")
